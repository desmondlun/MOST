...
public class JScrollPopupMenu extends JPopupMenu {
	...
	public static void main(String[] args) {
		...
		JXFrame win = new JXFrame("Test", true);
		Object[][] data = new Object[][]{
				new Object[]{"C1-R1", "C2-R1"},
				new Object[]{"C1-R2", "C2-R2"}
		};
		String[] cols = new String[]{"COL1", "COL2"};
		JScrollPopupMenu menu = new JScrollPopupMenu();
		//JXPopupMenu menu = new JXPopupMenu();
		JXTable table = new JXTable(data, cols);
		table.setComponentPopupMenu(menu);
		JXTree tree = new JXTree();
		tree.setComponentPopupMenu(menu);
		JXList list = new JXList(cols);
		list.setComponentPopupMenu(menu);
		JTabbedPane tabbedPane = new JTabbedPane();
		tabbedPane.add(table, "Table");
		tabbedPane.add(list, "List");
		tabbedPane.add(tree, "Tree");
		for (int i = 0; i < 100; i++) {
			menu.add(new JMenuItem("Test a very very very very very long string"));
